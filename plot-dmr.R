...
m1 = mat[(chrom == locs[,"chrom"]) & (dmr[1] < locs[,"end"]) & (dmr[2] > locs[,"start"]),]
lm1 = locs[(chrom == locs[,"chrom"]) & (dmr[1] < locs[,"end"]) & (dmr[2] > locs[,"start"]),]


dat = matrix(as.numeric(m1), nrow=nrow(m1))

colnames(dat) = colnames(m1)[1:ncol(m1)]
dat = t(dat) #[,shared])
